    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            */
            ...
            if (dt_doc.HasRows)
            {
                Console.WriteLine($"{"Id",-9} {"Ten tai khoan",-19} {"Mat khau",-19}");

                // đọc từng bản ghi
                while (dt_doc.Read())
                {
                    // cách 1:
                    // đọc dữ liệu ở cột có index = 0
                    var Id = dt_doc.GetInt32(0);

                    // cách 2:
                    // đọc dữ liệu ở cột có tên là "Username"
                    var TenTaiKhoan = dt_doc["Username"];

                    // cách 3:
                    // đọc dữ liệu ở cột có tên là "Password"
                    var MatKhau = dt_doc.GetString("Password");

                    Console.WriteLine($"{Id, -9} {TenTaiKhoan, -19} {MatKhau, -19}");
                }
            }
            else
            {
